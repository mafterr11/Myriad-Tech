...
const reduceTransition = (transition) => {
  ...
  if (typeof transition.delay === "number") {
    reducedTransition.delay = Math.min(transition.delay * 0.2, 0.1);
  }
...
const createResponsiveMotion = (Component, displayName) => {
  const ResponsiveMotion = forwardRef(({ variants, ...props }, ref) => {
    const isMobile = useIsMobile();
    const responsiveVariants = useMemo(
      () => (isMobile ? getMobileVariants(variants) : variants),
      [isMobile, variants],
    );
