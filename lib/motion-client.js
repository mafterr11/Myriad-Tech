...
const useIsMobile = () => {
  ...
  useEffect(() => {
    ...
    // Synchronize the lazy initial value in case the viewport changed between
    // hydration and effect setup.
...
const createResponsiveMotion = (Component, displayName) => {
  const ResponsiveMotion = forwardRef(
    ({ variants, viewport, mobileViewport, ...props }, ref) => {
      ...
      // forwardRef is the public contract of this wrapper; passing the opaque
      // ref through does not inspect ref.current during render.
